...
def _finding(
    ...
    if len(findings) < MAX_FINDINGS:
        findings.append(ScanFinding(code, severity, message, evidence or {}))
...
def _analyze_inventory(report: MediaInventoryReport, findings: list[ScanFinding]) -> None:
    ...
    videos = [stream for stream in report.streams if stream.codec_type == "video"]
    audios = [stream for stream in report.streams if stream.codec_type == "audio"]
    if len(videos) > 1:
        _finding(
            findings,
            "MULTIPLE_VIDEO_STREAMS",
            "REVIEW",
            "multiple video streams were found; review stream selection",
            {"video_stream_count": len(videos)},
        )
    if not audios:
        _finding(findings, "NO_AUDIO_STREAM", "INFO", "no audio stream was detected")
    ...
    for stream in videos:
        if stream.width is None or stream.height is None:
            ...
            _finding(
                ...
                {"stream_index": stream.index, "width": stream.width, "height": stream.height},
            )
...
def _analyze_qc(qc: dict[str, Any] | None, findings: list[ScanFinding]) -> None:
    if qc is None:
        return
    status = str(qc.get("status", "UNKNOWN"))
    if status == EvidenceStatus.FAIL.value:
        _finding(
            findings,
            "MEDIA_QC_FAILED",
            "FAIL",
            "media QC reported a failed condition",
            {"mode": qc.get("mode"), "reasons": qc.get("reasons", [])},
        )
    elif status in {EvidenceStatus.REVIEW.value, EvidenceStatus.UNKNOWN.value}:
        _finding(
            findings,
            "MEDIA_QC_INCOMPLETE",
            "REVIEW",
            "media QC is not sufficient for a final pass",
            {"mode": qc.get("mode"), "status": status},
        )


def _analyze_timeline(report: MediaTimelineReport, findings: list[ScanFinding]) -> None:
    checks = (
        (report.truncated, "TIMELINE_TRUNCATED", "timeline analysis reached its configured evidence bound"),
        (report.pts_non_monotonic, "PTS_NON_MONOTONIC", "presentation timestamps are non-monotonic"),
        (report.dts_non_monotonic, "DTS_NON_MONOTONIC", "decode timestamps are non-monotonic"),
        (report.duplicate_pts, "DUPLICATE_PTS", "duplicate presentation timestamps were detected"),
        (report.large_gaps, "LARGE_TIMESTAMP_GAPS", "large timestamp gaps were detected"),
    )
    for condition, code, message in checks:
        if condition:
            _finding(findings, code, "REVIEW", message)
...
def _analyze_decoder_regions(report: DecoderRegionReport, findings: list[ScanFinding]) -> None:
    if report.failed_window_count:
        _finding(
            ...
            {"failed_window_count": report.failed_window_count, "window_count": report.window_count},
        )
    ...
    *,
    ...
    if profile == "deep":
        ...
        if timeline.get("truncated"):
            return EvidenceStatus.REVIEW
        if any(bool(timeline.get(key)) for key in ("pts_non_monotonic", "dts_non_monotonic", "duplicate_pts", "large_gaps")):
            return EvidenceStatus.REVIEW
        if decoder_regions is not None and (
            decoder_regions.get("failed_window_count", 0)
            or not decoder_regions.get("sampling_complete", False)
        ):
            return EvidenceStatus.REVIEW
...
def run_forensic_scan(
    ...
    *,
    ...
    """Run a bounded composite forensic scan with conservative verdict semantics."""
    ...
    if not 1 <= decoder_max_windows <= 2048:
        raise ValueError("decoder_max_windows must be between 1 and 2048")
    if not 0 < decoder_window_seconds <= 60:
        raise ValueError("decoder_window_seconds must be between 0 and 60")
    ...
    _analyze_qc(inventory.qc, findings)

    timeline_report: MediaTimelineReport | None = None
    if profile in {"standard", "deep"}:
        try:
            timeline_report = analyze_media_timeline(artifact, timeout=timeout if timeout is not None else 1800.0)
    ...
    if decoder_errors:
        try:
            ...
            _finding(
                findings,
                "DECODER_ANALYSIS_FAILED",
                "REVIEW",
                "decoder-region analysis could not be completed",
                {"error": f"{type(exc).__name__}: {exc}"},
            )
    ...
    if not hash_stable:
        _finding(
            ...
            "FAIL",
            "artifact hash changed during forensic scanning; scan evidence is invalidated",
